#!/usr/bin/env python3
"""Filter a GPX track with the contextual fast+slow cascade model."""
...
def curve_to_delta(curve):
    """Convert cumulative slow error curve to per-timestep delta correction."""
...
def apply_context_cascade_filter(
    track_df,
    fast_model_path="models/model_final_context_cascade_v1_fast.keras",
    slow_model_path="models/model_final_context_cascade_v1_slow.keras",
    ...
    slow_model = load_model(slow_model_path)
    ...
    for i in range(0, sequence_length, max_sequence):
        ...
        slow_input = np.concatenate([input_data, fast_residual, fast_delta], axis=-1)
        slow_curve = slow_model.predict(slow_input, verbose=0)
        filtered_norm = fast_delta - curve_to_delta(slow_curve)
        filtered_chunks.append(filtered_norm[0, :chunk_len, :])
...
def parse_args():
    parser = argparse.ArgumentParser(description="Filter GPS track using contextual fast+slow cascade")
    parser.add_argument("input_gpx", help="Input GPX file")
    parser.add_argument("output_gpx", nargs="?", help="Output filtered GPX file")
    parser.add_argument("--fast-model", default="models/model_final_context_cascade_v1_fast.keras")
    parser.add_argument("--slow-model", default="models/model_final_context_cascade_v1_slow.keras")
    ...
    parser.add_argument("--suffix", default="nn_context_cascade_v1_filtered")
