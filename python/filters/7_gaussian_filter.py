#!/usr/bin/env python3
"""
Filtro Gaussiano - aplica suavizado usando una ventana gaussiana.
Proporciona un suavizado natural que preserva mejor las transiciones suaves.

Uso:
    python 7_gaussian_filter.py input_track.gpx [output_track.gpx]
    python 7_gaussian_filter.py input_track.gpx --sigma 1.5
    
Si no se especifica output_track.gpx, se genera automáticamente como:
    <directorio_entrada>/<nombre_original>_gaussian_filtered.gpx
"""
...
def create_gpx(lat, lon, ele, time=None, output_path="filtered_track.gpx"):
    ...
    gpx = ET.Element("gpx")
    gpx.set("version", "1.1")
    gpx.set("creator", "gaussian_filter")
    gpx.set("xmlns", "http://www.topografix.com/GPX/1/1")
    
    trk = ET.SubElement(gpx, "trk")
    name = ET.SubElement(trk, "name")
    name.text = "Gaussian Filtered Track"
...
def apply_gaussian_filter(track_df, sigma=1.5):
    """
    Aplica filtro gaussiano a las coordenadas.
    ...
    # Aplicar filtro gaussiano a cada coordenada
    filtered_df['lat'] = gaussian_filter1d(track_df['lat'].values, sigma=sigma)
    filtered_df['lon'] = gaussian_filter1d(track_df['lon'].values, sigma=sigma)
    filtered_df['ele'] = gaussian_filter1d(track_df['ele'].values, sigma=sigma)
    
    return filtered_df

def main():
    """Función principal."""
    parser = argparse.ArgumentParser(description='Apply Gaussian filter to GPS track')
    ...
    parser.add_argument('--sigma', type=float, default=1.5, help='Standard deviation for Gaussian kernel (default: 1.5)')
    parser.add_argument('--suffix', default='gaussian_filtered', help='Suffix for auto-generated output filename')
    ...
    try:
        ...
        filtered_df = apply_gaussian_filter(track_df, sigma=args.sigma)
        ...
        print(f"SUCCESS: Gaussian filtering completed successfully!")
