#!/usr/bin/env python3
"""Filter a GPX track with the contextual fast+slow residual cascade v3 model."""
...
def apply_context_cascade_filter(
    track_df,
    fast_model_path="models/model_final_context_cascade_v3_fast.keras",
    slow_model_path="models/model_final_context_cascade_v3_slow.keras",
    ...
    slow_input_matrix = np.concatenate([input_matrix, slow_input_matrix], axis=-1).astype(np.float32)
    ...
    for i in range(0, sequence_length, max_sequence):
        ...
        slow_input = np.zeros((1, max_sequence, slow_input_matrix.shape[1]), dtype=np.float32)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Filter GPS track using contextual fast+slow residual cascade v3")
    parser.add_argument("input_gpx", help="Input GPX file")
    parser.add_argument("output_gpx", nargs="?", help="Output filtered GPX file")
    parser.add_argument("--fast-model", default="models/model_final_context_cascade_v3_fast.keras")
    parser.add_argument("--slow-model", default="models/model_final_context_cascade_v3_slow.keras")
    ...
    parser.add_argument("--suffix", default="nn_context_cascade_v3_filtered")
