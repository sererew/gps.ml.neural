...
def create_family_groups(meta_dict):
    """
    Reagrupa pasadas por su número base para evitar data leakage.
    Ej: 4, 4a, 4b, 4c, 4d → familia '4'
        15, 15a, 15b, 15c, 15d → familia '15'
    """
    family_groups = {}
    
    for pasada in meta_dict.keys():
...
def validate_family_consistency(meta_dict, family_groups):
    """
    Valida que todas las pasadas de una misma familia estén en el mismo set
    para evitar data leakage.
    
    Returns:
        list: Lista de familias inconsistentes (vacía si todo está bien)
    """
    inconsistent_families = []
    
    for family_base, pasadas in family_groups.items():
        if len(pasadas) <= 1:
            continue  # Familia con una sola pasada, no hay problema
            
        # Verificar que todas las pasadas de la familia estén en el mismo set
        family_sets = {meta_dict[pasada]['set'] for pasada in pasadas}
        
        if len(family_sets) > 1:
            # Crear detalles de la inconsistencia
            set_details = {}
            for pasada in pasadas:
                set_name = meta_dict[pasada]['set']
                if set_name not in set_details:
                    set_details[set_name] = []
                set_details[set_name].append(pasada)
...
def find_base_family_pattern(pasada, family_groups):
    """
    Encuentra el patrón de la familia base para pasadas acortadas.
    
    Args:
        pasada: Nombre de la pasada (ej: '4a', '4b', '15c')
        family_groups: Dict de agrupamiento de familias
        
    Returns:
        str: Nombre de la pasada base que contiene el patrón (ej: '4', '15')
    """
    pasada_str = str(pasada)
    base_num = ''.join(filter(str.isdigit, pasada_str))
    
    if base_num in family_groups:
        family_members = family_groups[base_num]
        # Buscar el miembro base (sin letras) de la familia
        for member in family_members:
            if member == base_num:  # Es el número puro sin letras
                return member
        # Si no encontramos el número puro, usar el primer miembro ordenado
        return family_members[0]
    
    # Si no está en ninguna familia, usar la misma pasada
    return pasada
...
def generate_csvs_for_set(eligible_pasadas, meta_dict, stats, args, family_groups):
    ...
    for pasada in tqdm(eligible_pasadas, desc=f"Generando {args.set}"):
        ...
        if not pattern_files:
            print(f"⚠️  No se encontró patrón para familia base {pattern_pasada} (pasada {pasada})")
            continue
        
        if pattern_pasada != pasada:
            print(f"🔄 Pasada {pasada} usando patrón de familia base {pattern_pasada}")
        ...
        # Buscar grabaciones EN LA PASADA ORIGINAL (no en la base)
        recs = [p for p in glob.glob(os.path.join(pdir, "*_resampled.gpx"))
                if os.path.basename(p) != os.path.basename(trp_path)]
...
def main():
    ...
    # Crear grupos familiares para validar consistencia
    family_groups = create_family_groups(meta_dict)
    print(f"👨‍👩‍👧‍👦 Familias identificadas: {len(family_groups)}")
    for family_base, pasadas in family_groups.items():
        if len(pasadas) > 1:
            print(f"   Familia {family_base}: {pasadas}")
    
    # Validar consistencia de familias (prevenir data leakage)
    inconsistencias = validate_family_consistency(meta_dict, family_groups)
    if inconsistencias:
        print("❌ INCONSISTENCIAS ENCONTRADAS - Data leakage detectado!")
        for inc in inconsistencias:
            print(f"  - Familia {inc['family']}: {inc['pasadas']}")
            print(f"    Sets encontrados: {inc['set_details']}")
        print("\n💡 Todas las variantes de una familia (ej: 4, 4a, 4b, 4c, 4d) deben estar en el mismo set")
        return 1
    else:
        print(f"✅ Validación de familias correcta - Sin data leakage")
